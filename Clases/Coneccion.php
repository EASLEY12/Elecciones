		...
		if (is_array($objeto)) {
			...
			foreach ($objeto as $datos) {
				foreach ($datos as $registro ) {
					$tabla .= "<td>";
					$tabla .= utf8_encode($registro);
					$tabla .= "</td>";
				}
				if ($editar != "0") {
					//Recordar que aun no hago que modifique ni eliminine aun nose xk es este error si
					$tabla .= "<td><a href=".$form."Buscar.php?id=".$datos['Codigo'].">Modificar</a><img src='modificar.jpg'></dt>";
					$tabla .= "<td><a href=".$form."Borrar.php?id=".$datos['Codigo'].">Eliminar</a><img src='eliminar.jpg'></td>";
				}
				$tabla .= "</tr>";
			}
		 $tabla .= "</table>";	
	...
	if ($objeto != NULL) {
		if (is_array($objeto)) {
			...
			foreach ($objeto as $datos) {
				...
				if ($editar != "0") {
					//Recordar que aun no hago que modifique ni eliminine aun nose xk es este error si
					$tabla .= "<td><a href=".$form."Buscar.php?id=".$datos['id_partidos'].">Modificar</a><img src='modificar.jpg'></dt>";
					$tabla .= "<td><a href=".$form."Borrar.php?id=".$datos['id_partidos'].">Eliminar</a><img src='eliminar.jpg'></td>";
				}
				$tabla .= "</tr>";
			}
		 $tabla .= "</table>";	
...
function imprimetabla2($objeto,$form,$estilo="table",$editar=0){
	if ($objeto != NULL) {
		if (is_array($objeto)) {
			$tabla = "<table class=\"$estilo\">";
			$tabla .="<tr class='info'>";
